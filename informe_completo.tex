...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figuras/dominio_computacional.png}
\caption{Representación física del modelo mostrando el canal rectangular con dos obstáculos (vigas) y las condiciones de frontera aplicadas en cada región.}
\label{fig:dominio}
\end{figure}
...
\item Incógnitas (nodos internos): $105$
\end{itemize}
...
Esta optimización reduce significativamente el número de incógnitas de $250$ a $105$, acelerando cada iteración sin comprometer la captura de las características principales del flujo.
...
donde $\mathbf{X}$ es el vector de velocidades y $\mathbf{F}$ es el vector de residuos. El método de Newton-Raphson itera según:
...
\caption{Estructura dispersa de la matriz Jacobiana ($105 \times 105$) mostrando el patrón de cinco diagonales característico. Densidad: $\approx 4.76\%$.}
\label{fig:jacobiano}
\end{figure}
