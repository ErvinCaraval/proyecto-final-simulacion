...
def interpolate_cubic_natural_manual(V_low_res):
    """Applies 1D Cubic Spline interpolation (X then Y) for smooth visualization."""
...
def plot_solution(V_final, vy_value, method_name):
    ...
    ax.set_title(f'Solution: {method_name.upper()} (1D Natural Cubic Spline)')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    plt.tight_layout()
    
    filename = os.path.join(OUTPUT_DIR, f"result_spline_natural_1D_{method_name}.png")
