...
import time
from scipy import interpolate # Añadido para el Spline
import os
...
CARPETA_GRAFICAS = "graficas_V4_1"
...
# --- FUNCIÓN DE INTERPOLACIÓN BICÚBICA NATURAL (PARA VISUALIZACIÓN SUAVE) ---
def interpolate_bicubic_natural(V_low_res):
    """Aplica el Spline Bicúbico para suavizar la visualización."""
    # Los parámetros para interpolación deben ser definidos fuera de la clase
    ...
    V_high_res = interp_func(y_high, x_high)
    return V_high_res

# 4. FUNCIONES AUXILIARES
def plot_solution(V_final, vy_value, method_name):
    # Aplicar Interpolación Bicúbica Natural para Suavizar
    V_interpolated = interpolate_bicubic_natural(V_final)
    ...
    ax.set_title(f'Solución con: {method_name.upper()} (Visualización suavizada por Spline Bicúbico Natural)'); 
    ax.set_xlabel('X'); ax.set_ylabel('Y'); 
    plt.tight_layout(); 
    ruta_archivo = os.path.join(CARPETA_GRAFICAS, f"resultado_spline_{method_name}.png")
...
if __name__ == '__main__':
    ...
    print("\n--- GENERANDO GRÁFICOS (Spline Bicúbico Natural e SIN Spline) ---")
